...
CYN  "Nifty CLI binary installation script"
echo "---------------------------------------"
echo ""
...
case "$PROCESSOR" in
    arm* | aarch* | ppc* )
        if [ "$OS_FLAVOUR" != Darwin ]; then
            ...
            echo "  => $(CYN https://github.com/nifty-oss/cli)"
            echo ""
            echo "to build Nifty from the source code."
            exit 1
        fi
        ;;
...
if [ $SIZE -eq 0 ]; then
    RED "Aborting: could not download Nifty distribution"
    exit 1
fi
...
if [ ! "$(command -v $BIN)" = "" ]; then
    ...
    echo "Nifty binary was found at:"
    echo "  => $(CYN $EXISTING)"
    echo ""
    ...
    if [ "$(command -v $BIN)" = "" ]; then
        ...
        if [ -f "$ENV_FILE" ]; then
            ...
            echo "  => adding '$TARGET' to 'PATH' variable to execute 'nifty' from any directory."
...
# sanity check
if [ "$(command -v $BIN)" = "" ]; then
    # installation was completed, but Nifty is not in the PATH
